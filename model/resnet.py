...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm='bn'):
        ...
        self.bn1 = _normalization_3d(planes, norm)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3x3(planes, planes)
        self.bn2 = _normalization_3d(planes, norm)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm='bn'):
        ...
        self.bn3 = _normalization_3d(planes*4, norm)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv3d(
            ...
            padding=(3, 3, 3),
            bias=False)
        self.bn1 = _normalization_3d(64, norm)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool3d(kernel_size=(3, 3, 3), stride=(2,2,2), padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0], shortcut_type, norm=norm)
        self.layer2 = self._make_layer(
            block, 128, layers[1], shortcut_type, stride=2, norm=norm)
        self.layer3 = self._make_layer(
            block, 256, layers[2], shortcut_type, stride=2, norm=norm)
        self.layer4 = self._make_layer(
            block, 512, layers[3], shortcut_type, stride=2, norm=norm)
            ...
            if shortcut_type == 'A':
                ...
                downsample = nn.Sequential(
                    nn.Conv3d(
                        ...
                        kernel_size=1,
                        stride=stride,
                        bias=False), _normalization_3d(planes*block.expansion, norm))
